...
export const ui = {
  ...
  renderInvestmentSummary() {
    ...
    host.innerHTML = `
      <dt>Investissement camion</dt><dd>${withTooltip(
        formatCurrency(state.inputs.vehiclePrice),
        "Valeur TTC du camion frigorifique neuf à financer."
      )}</dd>
      <dt>Licences de transport</dt><dd>${withTooltip(
        formatCurrency(licensesCost),
        "Montant des licences communautaires (1800 € chacune) exigées par la DREAL."
      )}</dd>
      ...
      <dt>Financement bancaire demandé</dt><dd>${withTooltip(
        ...
        "Montant du prêt couvrant camion, licences et besoin en fonds de roulement (après déduction de l'apport)."
      )}</dd>
  ...
  renderSummaryOverview() {
    ...
    if (financingHost && monthly && loan) {
      ...
      financingHost.innerHTML = `
        <dt>Investissement camion</dt><dd>${formatCurrency(state.inputs.vehiclePrice)}</dd>
        <dt>Licences</dt><dd>${formatCurrency(licensesCost)}</dd>
        <dt>BFR (2 mois)</dt><dd>${formatCurrency(monthly.advanceTreasury)}</dd>
  ...
  renderExecutiveSummary() {
    ...
    target.textContent = `${companyName} est porté par ${managerName}, salarié de Frigo Est depuis 5 ans. Le client principal, ${mainClient || 'Frigo Est'}, garantit un flux régulier sous ${stabilityMap[contractStability]}. Avec un investissement de ${formatCurrency(state.inputs.vehiclePrice)} financé à hauteur de ${formatCurrency(loan.amount)} et une capacité d'autofinancement mensuelle de ${formatCurrency(monthly.caf)}, le projet démontre une couverture satisfaisante du service de la dette et une trésorerie positive dès les premières années.`;
  },
  ...
  renderRatios() {
    ...
    ratioList.innerHTML = `
      ...
      <dt>Amortissements annuels</dt><dd>${withTooltip(
        ...
        "Part de la valeur du camion passant en charge chaque année."
      )}</dd>
    ...
    if (ratios.marginNet && ratios.marginNet >= 0.12) {
      analysis.push("La marge nette dépasse 12 %, rassurant sur la rentabilité malgré les charges transport.");
    } else {
    ...
    if (ratios.loanToRevenue && ratios.loanToRevenue <= 0.8) {
      ...
      analysis.push("L'endettement représente plus de 80 % du CA, argumentez la solidité du contrat Frigo Est.");
    }
