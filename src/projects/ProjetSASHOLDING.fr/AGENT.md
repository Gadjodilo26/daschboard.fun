...
- Persistance : `localStorage` via `STORAGE_KEY = "atelier-entreprendre"` (voir `config/constants.js`). Utiliser `storage.save()` / `storage.load()` / `storage.export()`.
...
- Préférez modifier `calc.*` pour toute logique métier financier et `bookkeeping.js` pour les agrégations comptables. Évitez de mélanger avec `ui.*`.
